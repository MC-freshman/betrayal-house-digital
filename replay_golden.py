...
CASES: list[tuple[int, int, int | None]] = [
    ...
    (113, 3, 39),
    (113, 3, 40),
    # 40 号英雄胜线（M10-35 补）：另两条既有用例都是叛徒胜（伤害轨到 12），
    # 这条真正跑通"搜查→找到埋葬室→轮流挖 5/5"。
    (7, 5, 40),
    (113, 3, 41),
    (113, 3, 42),
    (113, 3, 43),
    ...
    (113, 3, 67),
    (109, 4, 60),
    # 68 号两条胜负线各钉一条：23/5p 实测集齐钥匙、开锁并逃出一半（英雄胜）；
    # 109/4p 实测被回合/伤害轨掷出 6+ 封死在迷宫里（叛徒胜）。
    (23, 5, 68),
    (109, 4, 68),
    # 69 号两种结局各钉一条：113/3p 实测追上并抓住小精灵（英雄胜）；109/4p 英雄全灭（叛徒胜）。
    #   「轨道 6 逃脱」在 bot 局几乎打不出来（小精灵速 5 不停跑、英雄还被孢子拖慢，
    #    往往先被反击磨死）——该胜负线由 verify_haunt69_catch_and_escape 专项兜住。
    (113, 3, 69),
    (109, 4, 69),
    # 70 号（收官本）两种叛徒胜利线各钉一条：113/3p 打光英雄（36 回合）、
    # 137/3p 完成转变（36 回合）。**已知局限**：4 人局（109/7/101 等种子）
    # 会跑成 400 回合僵局——英雄备武器线长 + 叛徒免疫 + 部分形态房间不可达，
    # 属平衡/难度问题（已修 chase_heroes/换层目标/备武器后直奔叛徒三项 bot
    # 行为），如实记录、不再假装解决。
    (113, 3, 70),
    (137, 3, 70),
    # —— M10-15 扩容：为 1–70 每本再补一条**不同人数**的用例。默认 5 人局
    #    (101/5p)；69 号/18 号 5 人局僵局或异常，改用 23/5p 与 131/4p。
    #    46 号的 5 人局曾崩在 `HauntAction` NameError（模块未 import engine
    #    却直接用了引擎的 dataclass）——本次扩容扫描抓出并已修。
    (101, 5, 1),
    (101, 5, 2),
    (101, 5, 3),
    (101, 5, 4),
