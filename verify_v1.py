...
def verify_generic_haunt_action_and_victory() -> None:
    # 用仍是模板骨架的 25 号剧本验证通用行动/轨道胜负
    # （1-24 号已精修，见 verify_haunt_systems 的对应用例）
    engine = GameEngine(seed=43)
    engine.start_new_game(_configs(4, "normal"))
    _trigger_specific_haunt(engine, 25)
    hero = next(player for player in engine.state.players if player.role == "hero")
    # 25 号骨架声明的房间列表里没有教堂，改用它在场的厨房
    ...
    assert any(action.id == "h25_hero_task" for action in actions)
    assert engine.perform_haunt_action(hero, "h25_hero_task")
