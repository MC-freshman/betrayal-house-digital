...
HAUNT_RULE_OVERRIDES: dict[int, dict[str, Any]] = {
    ...
    69: {
        ...
        #     → 英雄胜。
        #   已知简化：禁用神秘电梯/煤槽向上/坍塌房、屏障房两侧留孢子、免疫左轮
        #     （按属性免疫会误伤徒手速度攻击）、"先手"仅以轨道起点 1 体现（未改引擎回合顺序）。
        # 复校（2026-09-16，叛徒胜线排查）：修掉三处让"活到轨道 6"根本打不出来的规则错——
        #   · p151 "The wisp may explore new rooms as if it were an explorer" 已建模：
        #     逃不开时按"离英雄最远的空门口"翻一块新板块（同 47 号蛇头口径），
        #     只走 _place_room 不经 move_player，故"不抽牌、不因符号停步"自动满足。
        #   · on_monster_turn_start 原样返回 False → 逃跑结束后引擎默认怪物 AI 把
        #     小精灵往英雄方向拖（违反"不能进入已有孢子的房间""移动不受对手阻挡"）。
        #   · 移动力原写死 5 点，改为按 p17 投速度骰。
        #   实测（11 种子 × 3/4/5 人 = 33 局）：叛徒 22 / 英雄 11，其中 17 局走满
        #   轨道 6（改动前 33 局轨道最高 4、叛徒只靠"英雄全灭"赢 3 局）。
        "version": 3,
        "fidelity": "refined",
        "status": "playable",
        "mode": "wisp_capture",
